...
#### `alimtalk_send.send` · `bulk` 에 `webhook_url` 추가 (Ruby SDK parity)

발송 건별로 결과 웹훅을 받을 주소를 지정할 수 있다. 주면 발송 성공·실패·문자 대체발송·예약취소 웹훅이
**그 주소로만** 가고 프로젝트 웹훅 설정(`alimtalk_webhook.update`)은 쓰이지 않는다.
`https` 만 허용하며 2,000자를 넘으면 3028 로 거부된다. 서명은 프로젝트 시크릿으로 하므로,
설정 없이 시크릿만 필요하면 `alimtalk_webhook.rotate_secret` 으로 발급받는다.

* `send` — 이 한 건의 웹훅 주소다. ⚠️ 같은 `ref_id` 로 이미 접수·성공한 건을 다시 요청하면
  멱등 때문에 기존 접수가 그대로 돌아와 **새 주소는 무시된다**.
* `bulk` — 요청 단위 하나다(수신자별이 아니다). 이 요청으로 나간 모든 수신자 건의 결과가 그 주소로 간다.
  형식이 틀리면(`https` 아님·2,000자 초과) 요청 전체가 3028 로 거부된다.

생략하면 종전처럼 프로젝트 웹훅 설정을 따른다 — 기존 호출의 동작은 그대로다.
